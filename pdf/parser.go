...
func (parser *Parser) ReadObject(decryptor Decryptor) (Object, error) {
	...
	b, _ := parser.Peek(2)
	if len(b) == 0 {
		return KEYWORD_NULL, ErrReadError
	}
	...
	if b[0] == ']' {
		parser.Discard(1)
		return KEYWORD_NULL, ErrEndOfArray
	}
	...
	if b[0] == ')' {
		parser.Discard(1)
		return KEYWORD_NULL, ErrEndOfString
	}
	...
	if string(b) == ">>" {
		parser.Discard(2)
		return KEYWORD_NULL, ErrEndOfDictionary
	}
	...
	if b[0] == '>' {
		parser.Discard(1)
		return KEYWORD_NULL, ErrEndOfHexString
	}
...
func (parser *Parser) ReadDictionary(decryptor Decryptor) Dictionary {
	...
	for {
		...
		if err == ErrReadError || err == ErrEndOfDictionary {
			break
		}
	...
	for open_parens := 1; true; {
		...
		if b == '\\' {
			...
			if b >= '0' && b <= '7' {
				...
				val, err := strconv.ParseUint(code.String(), 8, 8)
				if err != nil {
